...
_HOSTED_PROFILE_ALIASES: dict[str, tuple[str, str]] = {
    ...
    "motionjson-colab-sam2-session": ("sam2-hosted", "motionjson-colab-sam2-session"),
    "sam2-hosted:motionjson-colab-sam2-session": ("sam2-hosted", "motionjson-colab-sam2-session"),
    ...
    "motionjson-colab-sam3-session": ("sam3-hosted", "motionjson-colab-sam3-session"),
    "sam3-hosted:motionjson-colab-sam3-session": ("sam3-hosted", "motionjson-colab-sam3-session"),
}
...
_PROVIDER_REGISTRY: list[dict[str, Any]] = [
    ...
    _entry(
        ...
        aliases=[
            ...
            "motionjson-colab-sam2-session",
            "hosted-sam2",
            "sam2-hosted:replicate-sam2-video",
            "sam2-hosted:custom-sam2-compatible",
            "sam2-hosted:motionjson-colab-sam2-session",
        ],
    ...
    _entry(
        "evolink",
        aliases=["evolink-settings", "evolink-ai"],
        label="EvoLink settings",
        description="Server-side EvoLink credentials and model selection for OpenAI-compatible planning.",
        kind="llm_provider",
        locality="hosted",
        implemented=True,
        worker_eligible=False,
        credential_fields=[{"name": "api_key", "env": "EVOLINK_API_KEY", "required": True}],
        endpoint_fields=[{"name": "base_url", "env": "EVOLINK_BASE_URL", "required": False}],
        model_options=[
            {"id": "gpt-5.2", "label": "GPT-5.2"},
            {"id": "gpt-5.1", "label": "GPT-5.1"},
            {"id": "gemini-3.1-flash-lite-preview", "label": "Gemini 3.1 Flash Lite Preview"},
            {"id": "deepseek-v4-flash", "label": "DeepSeek V4 Flash"},
            {"id": "__custom__", "label": "Custom EvoLink model id"},
        ],
        default_model="gpt-5.2",
        remediation="EvoLink planning is server-side only and requires explicit hosted opt-in.",
    ),
    _entry(
        "openai",
    ...
    _entry(
        "evolink-planner",
        aliases=["evolink_planner"],
        label="EvoLink planner",
        description="Server-side EvoLink OpenAI-compatible planning connector; output is a proposed run config only.",
        kind="planning_provider",
        locality="hosted",
        implemented=True,
        worker_eligible=False,
