...
def normalize_benchmark_modes(value: str | list[str] | tuple[str, ...] | None) -> list[str]:
    aliases = {
        ...
        "class": "class_detector_mock",
        "classes": "class_detector_mock",
        "class-mock": "class_detector_mock",
        "class-detector-mock": "class_detector_mock",
        "yolo-mock": "class_detector_mock",
        "text-mock": "text_detector_mock",
        "threshold": "external_masks",
    }
    allowed = {"external_masks", "motion_foreground", "text_detector_mock", "sam_auto_mock", "class_detector_mock"}
...
def _mode_label(mode: str) -> str:
    return {
        ...
        "class_detector_mock": "Mock class detector",
...
def _run_pipeline_for_mode(
    *,
    ...
    if mode == "class_detector_mock":
        labels = [item["label"] for item in fixture_manifest["objects"]] or ["object"]
        return run_multi_object_pipeline(
            video_path=video_path,
            out_dir=run_dir,
            object_specs=[],
            candidate_provider=ClassDetectorDiscoveryProvider(),
            candidate_config={
                "mock": True,
                "class_preset": "custom",
                "classes": labels,
                "max_candidates": len(labels),
                "write_box_masks": True,
            },
            candidate_to_specs=lambda candidates: object_specs_from_candidates(candidates, base_dir=run_dir),
            sample_fps=sample_fps,
            max_frames=max_frames,
