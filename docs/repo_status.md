...
| Local SAM3 execution | Optional. Diagnostics report `sam3`, Python runtime, torch/CUDA, and `SAM3_LOCAL_MODEL` status. Mock SAM3 discovery modes are available for UI/API smoke checks; real local SAM3 uses the optional adapter only when diagnostics pass. |
