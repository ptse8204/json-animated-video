...
| Local SAM2 prompt tracking | You want point/box-prompted object tracing and want frames to stay on your machine. In the UI choose `Cut out one object` and `Model setup` -> `SAM2 prompt tracking`. | 16 GB RAM minimum for small clips, 32 GB recommended. NVIDIA GPU with 8 GB VRAM can handle small experiments; 12-16+ GB VRAM is more comfortable. Requires SAM2 package, torch/CUDA, checkpoint path, and config path. CPU is expected to be slow. | None after setup. |
| SAM2 HF automatic masks | SAM3 Scene Sweep is blocked and you need an automatic-mask fallback for `Find everything in scene`. | Torch/Transformers runtime and cache space for `facebook/sam2.1-hiera-large`. Does not require the official `sam2` package/checkpoint/config setup. | None after setup. Model download requires explicit UI network/disk confirmation. |
| Hosted SAM2 | You want SAM2-style tracing without a local GPU. In `Model setup`, use `Replicate SAM2 video` or a custom SAM2-compatible endpoint after `Change model`. | No local GPU or model weights. Requires provider SDK/dependency only when the selected profile needs one. | Requires an API key, hosted-call opt-in, and cost/privacy acknowledgement before network tests or hosted runs. Frames and prompts may leave the machine. |
| SAM3 Scene Sweep | You want `Find everything in scene`. In `Model setup`, use `SAM3 Scene Sweep`. | Torch/Transformers runtime, model cache space, and enough CPU/GPU memory for the clip. Default `sam3TrackerModel` is `facebook/sam3`; a local Hugging Face model directory is allowed. Do not use a single `sam3.pt` file for this path. | None after setup. Model download requires explicit UI network/disk confirmation and upstream model access when gated. |
| Official local SAM3 concept/exemplar | You want advanced concept/exemplar discovery locally and have a strong CUDA machine. | 32 GB RAM recommended. Modern NVIDIA GPU with 16+ GB VRAM recommended unless your exact checkpoint/clip has been verified. Requires official SAM3 package, a real local `sam3.pt`, torch/CUDA, and compatible Python runtime. | None after setup. Model checkpoint access may require upstream approval. |
| Hosted SAM3 | You want text/concept segmentation without local SAM3 hardware. In `Model setup`, use `Roboflow SAM3`, `Fal SAM3 image`, or a custom SAM3 endpoint. | No local GPU or checkpoint. | Requires an API key, hosted-call opt-in, and cost/privacy acknowledgement. Frames, prompts, or derived image data may be sent to the selected provider. Hosted scene sweep is enabled only when the profile explicitly advertises automatic masks. |
