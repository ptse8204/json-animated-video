...
const requiredProductIds = [
  ...
  "mainJobCenter",
  "studioReviewPanel",
  "studioTrackInspector",
  "studioExportCard",
  "studioExportReuseGuide",
  "reviewCorrectionSectionSlot",
  "providerWarning",
  "runPlanAlert",
  "viewerStage",
  "previewVideo",
  ...
  "relabelTrackButton",
  "studioValidateExportButton",
  "studioExportMotionJsonButton",
  "artifactBrowser",
  "fallbackDiagnostics",
  "routeList",
];

// Temporary compatibility shims used by keyboard tests, local storage restore,
// and older backend adapters. They must stay hidden or secondary in the UI.
const compatibilityShimIds = [
  "workflowStepper",
  "studioProgressStepper",
  "workspaceSidebar",
  "sidebarToggle",
...
for (const id of [...requiredProductIds, ...compatibilityShimIds]) {
  if (!index.includes(`id="${id}"`)) {
    throw new Error(`index.html is missing #${id}`);
  }
}

for (const stage of ["goal", "source", "target", "model", "preflight", "run", "review", "correct", "export", "reuse"]) {
  if (!new RegExp(`data-stage-root="[^"]*\\b${stage}\\b`).test(index)) {
    throw new Error(`index.html is missing data-stage-root="${stage}"`);
  }
}
